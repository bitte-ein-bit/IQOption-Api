...
class Position():
    ...
    def __parse_data(self, data):
        orders = None
        if ("orders" in data and len(data["orders"]) == 0) or "orders" not in data and hasattr(self, 'orders'):
            orders = self.orders
    ...
    def update(self, data):
        ...
        if self.is_open() != prev:
            ...
            logger.info('{},{},{},{},{},{},{},{},{},{},{}'.format(self.update_at, self.create_at, self.close_at, self.sell_avg_price_enrolled, self.buy_avg_price_enrolled, self.leverage, self.margin, self.count, self.close_effect_amount_enrolled, self.close_reason, self.instrument_id))
    ...
    def update_order(self, data):
        ...
        if data["type"] == "stop" and data["status"] != "canceled":
            self.logger.debug("updated stop_lose_order_id")
            self.stop_lose_order_id = order_id
        if data["type"] == "limit" and data["status"] != "canceled":
            self.logger.debug("updated take_profit_order_id")
    ...
    def get_stoploss(self, percent_buffer, current_price):
        if self.is_sell():
            return current_price + percent_buffer * self.get_open_price() / self.leverage
        elif self.is_buy():
            return self.get_open_price() / ((self.get_open_price() / current_price) + (percent_buffer / self.leverage))
        else:
